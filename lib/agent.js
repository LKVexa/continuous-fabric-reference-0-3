...
const {atomic,sha,adapter,payload,OPS}=require('./common');
...
function startAgent(config,{onState=()=>{},executor=execute}={}) {
  ...
  if(!Array.isArray(config.operations)||!config.operations.length||config.operations.length>OPS.length||config.operations.some(op=>!OPS.includes(op)))throw new Error('BAD_AGENT_OPERATIONS');
  if(typeof config.stateFile!=='string'||!config.stateFile.length)throw new Error('BAD_STATE_FILE');
  const stateFile=path.resolve(config.stateFile), lock=stateFile+'.lock';
  fs.mkdirSync(path.dirname(lock),{recursive:true});
  const lockFd=fs.openSync(lock,'wx',0o600);fs.writeFileSync(lockFd,String(process.pid));fs.closeSync(lockFd);
  let cache;
  try {
    if(fs.existsSync(stateFile)&&fs.statSync(stateFile).size>40*1024*1024)throw new Error('RECEIPT_CACHE_LIMIT');
    cache=fs.existsSync(stateFile)?JSON.parse(fs.readFileSync(stateFile,'utf8')):{};
    if(!cache||typeof cache!=='object'||Array.isArray(cache)||Object.keys(cache).length>1000)throw new Error('BAD_RECEIPT_CACHE');
    for(const [key,r] of Object.entries(cache)) {
      if(!UUID.test(key)||!r||r.id!==key||!UUID.test(r.lease)||!['EXECUTING','SUCCEEDED','FAILED','UNKNOWN'].includes(r.state)||typeof r.acked!=='boolean'||! /^[0-9a-f]{64}$/.test(r.fingerprint))throw new Error('BAD_RECEIPT_CACHE');
  ...
  const connect=()=>{
    ...
    ws.addEventListener('open',()=>{send({type:'register',operations:config.operations});});
    ...
    ws.addEventListener('message',async e=>{
      ...
      try {
        ...
        if(m.type==='receipt.ack'){
          if(!ready||!UUID.test(m.id)||!UUID.test(m.lease))throw new Error('BAD_RECEIPT_ACK');
          const r=Object.hasOwn(cache,m.id)?cache[m.id]:null;
          if(r&&r.lease===m.lease){r.acked=true;persist();}return;
        }
        ...
        if(busy||Object.keys(cache).length>=1000||j.deadline<Date.now()||!config.operations.includes(j.op))throw new Error('JOB_REFUSED');
  ...
  try { connect(); } catch(e) {fs.unlinkSync(lock);throw e;}
