...
The internal worker bridge uses POST /internal/fabric with an ephemeral gateway secret and trusted worker principal fields. It rejects external socket addresses, browser Origin headers, missing credentials, malformed inputs and oversized bodies. It is not a public REST API.
