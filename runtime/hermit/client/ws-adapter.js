    ...
    return {
      ...
      async status(sessionId) {
        ...
        return { sessionId, cwd: '', cols: tab.cols, rows: tab.rows, busy: false, nodes: [], dfRoot: null, photon: { url: null, token: false }, remote: true,
          connection: rec && rec.ws.readyState === 1 ? 'connected' : 'disconnected', unacknowledgedInputs: rec ? Math.max(0, rec.inSeq - rec.executedSeq) : 0, creditRemaining: rec ? rec.creditBytes - rec.consumedBytes : 0, epoch: rec ? rec.epoch : null, build: config && config.build, cfp: !!(config && config.capabilities.cfp), fabric: !!(config && config.capabilities.fabric) };
      },
