...
const { parse, expandWord } = require('./pipeline');
...
class SpiralKernel extends EventEmitter {
  ...
  _banner(session) {
    if (this.host.banner) return this.host.banner(session);
  ...
  async _executeLine(session, line) {
    let segments;
    try { segments = parse(line, session.env, { deferExpansion: true }); }
    catch (e) { session.env.set('?', '2'); throw e; }
    let lastCode = 0;
    for (const seg of segments) {
      ...
      try { lastCode = await this._runPipeline(session, seg.pipeline); }
      catch (e) { session.env.set('?', '1'); throw e; }
  ...
  async _runPipeline(session, pipeline) {
    ...
    try {
      for (let k = 0; k < pipeline.length; k++) {
        const source = pipeline[k];
        const cmd = { argv: source.argv.map(t => expandWord(t, session.env)), redirects: source.redirects.map(r => ({ ...r, target: expandWord(r.target, session.env) })) };
