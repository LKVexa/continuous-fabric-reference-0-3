...
(function () {
  ...
  async function pollStatus() {
    ...
    if (st.remote) {
      ...
      $('st-fabric').querySelector('.k').textContent = st.cfp ? 'CFP' : 'DF';
      $('st-nodes').textContent = st.cfp ? 'hub: ' + st.connection : (st.fabric ? 'enabled' : 'disabled');
      $('st-fabric').title = st.cfp ? 'Terminal connection to the hub. Use cfp nodes for live agent status.' : 'Native DF capability';
      $('st-dfroot').textContent = 'virtual shell'; $('st-photon').style.display = 'none';
      $('st-version').textContent = st.build ? 'CFP v' + st.build.version : 'HERMIT';
      $('st-version').title = st.build ? 'HERMIT ' + st.build.terminalVersion + ' · hermit.vws.v2' : 'hermit.vws.v2';
      const tabR = tabs.get(active); if (tabR) tabR.busy = false; return;
    }
